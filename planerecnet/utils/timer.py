...
# Orignal code comes from https://github.com/dbolya/yolact
# Changed the old timer library, and utilized torch.cuda.Event() for accurate timing in GPU
# Example:
# import timer
# timer.reset()
...
def stop(fn_name=None, use_stack=True):
    ...
    if use_stack:
        ...
        if _start_times[fn_name] > -1:
            ender.record()
            ender.synchronize()
            _total_times[fn_name] += starter.elapsed_time(ender)
        else:
...
def print_stats():
    ...
    for name in all_fn_names:
        print(format_str.format(name, _total_times[name]))

    print(sep_text)
    print(format_str.format("Total", total_time()))
    print()
