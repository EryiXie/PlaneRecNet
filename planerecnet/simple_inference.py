...
from planerecnet.models.functions.funcs import PCA_svd
...
def parse_args(argv=None):
    ...
    parser.add_argument("--nms_mode", default="matrix", type=str, choices=["matrix", "mask"],
                        help='Choose NMS type from matrix and mask nms.')
    ...
    parser.add_argument("--depth_mode", default="colored", type=str, choices=["colored", "gray"],
                        help='Choose visualization mode of depth map')
        ...
        if not no_text or not no_box:
            for j in reversed(range(num_dets)):
                ...
                if not no_text:
                    ...
                    cv2.putText(frame_numpy, text_str, text_pt, font_face, font_scale, text_color, font_thickness,
                                cv2.LINE_AA)
        if not no_text:
            score = pred_scores[j].detach().cpu().numpy()
            _class = cfg.dataset.class_names[pred_classes[j].cpu().numpy()]
...
def inference_image(net: PlaneRecNet, path: str, save_path: str = None, depth_mode: str = 'colored'):
    ...
    blended_frame, depth = display_on_frame(results[0], frame, no_mask=args.no_mask, no_box=args.no_box,
